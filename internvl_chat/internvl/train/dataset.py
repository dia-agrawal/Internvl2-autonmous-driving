...
def preprocess_internvl2_5(
    ...
    if not text_only:
        ...
        current_image_idx = 0
        for conversation in conversations:
            # Treat 'user' as 'human' to accept alternate role labels in dataset
            if conversation.get('from') in ('human', 'user'):
                # Support both '<image>' and legacy '<img>' tags
                image_cnt = conversation.get('value', '').count('<image>') + conversation.get('value', '').count('<img>')
                for i in range(image_cnt):
                    ...
                    # Prefer replacing '<image>' first, then '<img>' if present
                    if '<image>' in conversation['value']:
                        conversation['value'] = conversation['value'].replace('<image>', image_tokens, 1)
                    else:
                        conversation['value'] = conversation['value'].replace('<img>', image_tokens, 1)
                    current_image_idx += 1
            new_conversations.append(conversation)
        conversations = new_conversations
        if current_image_idx != num_image:
            print(f'WARNING: expected {num_image} images but found {current_image_idx} in sources for ds {ds_name}')
            sys.stdout.flush()
    ...
    for conversation in conversations:
        ...
        if role_from in ('human', 'user'):
            ...
            raise NotImplementedError(f'Unknown role: {role_from}')
