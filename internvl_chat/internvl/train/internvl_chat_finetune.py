...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(
        ...
        if self.group_by_length:
            ...
            self.length = []
            for idx, raw_line in enumerate(self.raw_data):
                try:
                    data_item = json.loads(raw_line)
                except Exception as e:
                    raise ValueError(
                        f"Failed to parse JSON on line {idx} of {meta['annotation']}: {e}\n" \
                        f"Offending line (first 300 chars): {raw_line[:300]!r}"
                    )

                if 'length' in data_item:
                    token_length = data_item['length']  # Use precomputed length if available
                else:
                    # Validate required key exists and provide an actionable error if not
                    if 'conversations' not in data_item:
                        raise ValueError(
                            f"Missing required key 'conversations' in JSONL record on line {idx} of {meta['annotation']}. "
                            f"Ensure each record contains a 'conversations' list. Offending record preview: {str(data_item)[:400]}"
                        )
                    ...
                    try:
                        ...
                        raise ValueError(
                            f"Error extracting 'conversations' values on line {idx} of {meta['annotation']}: {e}. "
                            f"Offending record preview: {str(data_item)[:400]}"
                        )
    ...
    def get_preprocess_function(self):
        ...
        name = str(self.template_name).lower()
        if name == 'hermes-2':
            preprocess_function = preprocess_mpt
        elif name == 'internlm2-chat':
            preprocess_function = preprocess_internlm
        elif name == 'phi3-chat':
            preprocess_function = preprocess_phi3
        # Map all internvl* conv styles (including internvl3) to the robust internvl2_5 preprocessor
        elif name == 'internvl2_5' or name.startswith('internvl'):
...
def main():
    ...
    if model_args.model_name_or_path is not None:
        ...
        # Only enable flash attention if the package is available; otherwise fall back to eager
        try:
        ...
        if config.llm_config.model_type == 'internlm2':
            if _HAS_FLASH_ATTN:
                setattr(config.llm_config, 'attn_implementation', 'flash_attention_2')  # for InternLM
                logger.info('Using flash_attention_2 for InternLM')
            else:
                setattr(config.llm_config, 'attn_implementation', 'eager')
                logger.info('flash_attn not found: using eager attention for InternLM')
        else:
            if _HAS_FLASH_ATTN:
                setattr(config.llm_config, '_attn_implementation', 'flash_attention_2')  # for LLaMA
                logger.info('Using flash_attention_2 for LLaMA')
            else:
                setattr(config.llm_config, '_attn_implementation', 'eager')
                logger.info('flash_attn not found: using eager attention for LLaMA')
