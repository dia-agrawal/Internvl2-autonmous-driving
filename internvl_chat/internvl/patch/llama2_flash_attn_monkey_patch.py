...
import torch
# Try to import flash_attn, but don't hard-fail if missing. If missing,
# set a flag and leave the module able to be imported; functions that rely
# on flash_attn will be no-ops or skipped by the caller.
try:
...
def replace_llama2_attn_with_flash_attn():
    if not HAS_FLASH_ATTN:
        print(
            'flash_attn not found: skipping LLaMA2 flash-attn monkey patch (using eager attention).'
        )
        return
